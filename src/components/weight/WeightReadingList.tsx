...
import type { AppSettings, TagDefinition, WeightReading } from '../../lib/types';

type WeightReadingListProps = {
  title: string;
...
export function WeightReadingList({
  title,
  readings,
  tagsById,
  settings,
...
}: WeightReadingListProps) {
  return (
    <section className="list-card" aria-labelledby="weight-list-title">
      <div className="section-header-inline">
        <div>
          <h2 id="weight-list-title">{title}</h2>
          ...
          {readings.map((reading) => {
            ...
            const accentColor =
              classification.status === 'inside'
                ? settings.weightChartColorInside
                : classification.status === 'outside'
                  ? settings.weightChartColorOutside
                  : settings.weightChartColorNeutral;
            ...
            return (
              ...
              >
                <div className="reading-card__topline">
                  <div>
                    <p className="reading-card__value">{reading.value} kg</p>
                ...
                <div className="action-row compact">
                  <Link to={`/weight/edit/${reading.id}`} className="secondary-button small">Edit</Link>
                </div>
              </article>
            );
          })}
